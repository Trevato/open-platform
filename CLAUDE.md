...
```
                    ┌─── Cloudflare Tunnel (optional) ───► *.{PLATFORM_DOMAIN}
                    │
...
Multiple domains can be served from a single platform instance. Configure in `open-platform.yaml`:
...
- **Org-domain mapping**: Assign domains to orgs via `PUT /api/v1/orgs/:name/domain`. Apps in that org deploy on the assigned domain.
- **Woodpecker org secrets**: Automatically provisioned when apps are created via `POST /api/v1/platform/apps`. All 8 CI secrets (registry credentials, domain, push host, etc.) are set per-org.
- **Repo website URL**: Set automatically on app creation to `https://{app}.{org-domain}`.
- **Cloudflare routes**: Each extra domain needs a `*.{domain}` wildcard route on the tunnel.
- **CoreDNS**: Bootstrap generates zone entries for each domain.

### Preview Environments

PR previews deploy at `pr-N-<app>.{PLATFORM_DOMAIN}`, protected by OAuth2-Proxy (Forgejo login required):

- **Namespace**: `op-{org}-{repo}-pr-{number}` (e.g., `op-system-social-pr-1`)
- **Domain**: `pr-{number}-{repo}.{PLATFORM_DOMAIN}` (e.g., `pr-1-social.example.com`)
...
Configured via `tls.mode` in `open-platform.yaml`:
