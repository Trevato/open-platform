...
| `AUTH_SECRET` | next-auth session encryption key | (generate with `openssl rand -base64 32`) |
| `AUTH_FORGEJO_ID` | Forgejo OAuth2 client ID | |
| `AUTH_FORGEJO_SECRET` | Forgejo OAuth2 client secret | |
| `NEXTAUTH_URL` | Public app URL | `https://PROJECT_NAME.dev.test` |
| `NODE_EXTRA_CA_CERTS` | Path to platform CA cert | `/etc/ssl/certs/openplatform.crt` |
...
|----------|---------|-------|
| `deploy.yaml` | Push to `main` | Kaniko build, push to Forgejo registry, apply schema, deploy |
| `test.yaml` | Pull request | Install, typecheck, lint, test |
...
Forgejo OAuth2 via next-auth.

1. Go to **Forgejo** > **Site Administration** > **Applications** (or user settings)
2. Create OAuth2 app with redirect URL: `https://PROJECT_NAME.dev.test/api/auth/callback/forgejo`
...
3. **Create OAuth2 app** -- in Forgejo with redirect URL `https://PROJECT_NAME.dev.test/api/auth/callback/forgejo`
