...
export function InstanceServiceList({ slug }: { slug: string }) {
  const [services, setServices] = useState<ServiceStatus[]>([]);
  const [loading, setLoading] = useState(true);

  const fetchServices = useCallback(async () => {
    try {
      const res = await fetch(`/api/instances/${slug}/services`);
      if (!res.ok) return;
  ...
  if (services.length === 0) {
    return (
      <div className="empty-state">
        ...
        <p>Platform services will appear here once the instance is running.</p>
      </div>
    );
  }
