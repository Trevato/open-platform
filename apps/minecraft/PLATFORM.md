...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string | `postgresql://user:pass@postgres-rw.postgres.svc.cluster.local:5432/minecraft` |
...
| `MINECRAFT_NAMESPACE` | Kubernetes namespace for server management | `minecraft` |
...
Every PR gets an isolated preview at `pr-{N}-minecraft.dev.test` with:
...
- **Callback URL**: `https://minecraft.dev.test/api/auth/oauth2/callback/forgejo`

### Setup

1. Create a Forgejo OAuth2 app with redirect URL: `https://minecraft.dev.test/api/auth/oauth2/callback/forgejo`
...
3. **Create OAuth2 app** -- in Forgejo with redirect URL `https://minecraft.dev.test/api/auth/oauth2/callback/forgejo`
...
The minecraft app uses a dedicated `minecraft-manager` ServiceAccount with a Role granting permissions to manage deployments, services, and pods within its namespace. This enables the app to create and manage Minecraft server pods directly via the Kubernetes API.
