...
/** List all cluster nodes with status, capacity, and pod counts. */
export async function listNodes(): Promise<NodeInfo[]> {
  ...
  return (nodeList.items || []).map((node) => {
    ...
    const internalAddr = addresses.find((a) => a.type === "InternalIP");

    return {
      name: node.metadata?.name || "",
      status: readyCondition?.status === "True" ? "Ready" : "NotReady",
      role: labels["open-platform.sh/node-role"] || "unknown",
      ...
      internalIp: internalAddr?.address || "",
      kubeletVersion: node.status?.nodeInfo?.kubeletVersion || "",
    };
  });
}

/** Patch node metadata labels. Null values remove the label. */
...
export async function deleteNode(name: string): Promise<void> {
  await coreV1.deleteNode({ name });
}

/** Read the k3s join token and server URL. Returns null if not on a server node. */
export async function getJoinToken(): Promise<{
  token: string;
  serverUrl: string;
} | null> {
  try {
    ...
    // Determine server URL from the first node's internal IP
    const nodeList = await coreV1.listNode();
    const firstNode = nodeList.items?.[0];
    const addresses = firstNode?.status?.addresses || [];
    const internalAddr = addresses.find((a) => a.type === "InternalIP");
    const serverUrl = `https://${internalAddr?.address || "127.0.0.1"}:6443`;
